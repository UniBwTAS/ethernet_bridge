...
![Status Badge](https://github.com/UniBwTAS/ethernet_bridge/actions/workflows/basic-build-ci.yml/badge.svg?branch=master)

ethernet_bridges are a set of ROS nodes to bridge network interfaces from and to the ROS message definitions `ethernet_msgs`. The Ethernet bridge can be used to insert network abstraction layers in ROS processing chains and to separate Ethernet interfaces from parser nodes. As Ethernet data is usually compressed, it is conceivable to store raw Ethernet data in ROS bags instead of processed or parsed device data. By this means, the size of ROS bags is kept minimal while providing a longterm-stable, generic data interface.
...
- UDP node
- In development: high-bandwidth UDP node which bundles UDP packets, TCP client node
- Bidrectional online transfers
...
```
  ...
  <arg name="ethernet_listenIp"     default="0.0.0.0" />
  ...
  <node pkg="ethernet_bridge" type="udp" name="udp">
    ...
    <param name="ethernet_bindAddress"  value="$(arg ethernet_listenIp)" />
...
We look forward to contributions to `ethernet_bridge` and `ethernet_mgs`. Please note that modifications on existing `ethernet_msgs` definitions would impair longterm stability and are avoided.